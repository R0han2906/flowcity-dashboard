...
import { Mail, Lock, Train, ArrowRight, Eye, EyeOff, Loader2 } from "lucide-react";
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 md:p-12 h-screen overflow-y-auto hide-scrollbar">
        <div className="max-w-[480px] w-full">
          <div className="mb-6 lg:mb-8 text-center sm:text-left">
            ...
            <p className="text-sm text-gray-500 font-medium mt-2">Log in to your FlowCity account</p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <motion.div variants={fadeUp}>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} className="w-full pl-12 pr-12 py-3 bg-gray-50 rounded-2xl border border-gray-200 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 transition-all" placeholder="Enter password" value={password} onChange={e => setPassword(e.target.value)} />
              ...
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-4 pt-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-[#1b3a2a] focus:ring-[#1b3a2a] accent-[#1b3a2a]" />
              ...
              <motion.button variants={fadeUp} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                disabled={isLoading}
                className="w-full mt-6 bg-[#1b3a2a] text-white rounded-2xl py-4 font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:shadow-lg disabled:opacity-70 transition-all">
                {isLoading ? <Loader2 size={18} className="animate-spin" /> : <>Log In <ArrowRight size={18} /></>}
              </motion.button>
              
              <motion.div variants={fadeUp} className="relative py-4 mt-2 flex items-center justify-center">
          ...
          <p className="text-center text-sm font-medium text-gray-500 mt-8">
            Don't have an account? <Link to="/signup" className="text-[#1b3a2a] font-bold hover:underline">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
